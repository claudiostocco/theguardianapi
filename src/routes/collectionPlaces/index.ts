...
import { CollectionPlace } from "../../types/collectionPlaces";
...
collectionPlacesRouter.get('/:address', async (req, res) => {
...
collectionPlacesRouter.get('/:neighborhood', async (req, res) => {
...
collectionPlacesRouter.put('/:id', async (req, res) => {
    if ((req.params.id) && (req.body)) {
        const collectionPlace = req.body as CollectionPlace;
        if (collectionPlace) {
            const updated = await update('collectionPlaces', {_id: req.params.id}, collectionPlace);
...
collectionPlacesRouter.delete('/:id', async (req, res) => {
    if (req.params.id) {
        const deleted = await remove('collectionPlaces', {_id: req.params.id});
